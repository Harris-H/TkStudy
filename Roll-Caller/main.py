...
def takeone():
    with open('库.txt', encoding='utf-8') as a:
        contents = a.read()
...
def author():
    messagebox.showinfo("作者", "Authors : Herio")
...
if __name__ == '__main__':
    ...
    var0.set("请选择人数")
    try:
        with open('库.txt', encoding='utf-8') as ku:
            contents = ku.read()
    except FileNotFoundError:
        var0.set("请导入人名列表")
    ...
    theButton1 = Button(root, text="1人", command=takeone, font=('Arial', 17), height=1).grid(row=4, column=0, padx=20)

    theButton2 = Button(root, text="2人", command=taketwo, font=('Arial', 17), height=1).grid(row=4, column=1)

    theButton3 = Button(root, text="3人", command=takethree, font=('Arial', 17), height=1).grid(row=4, column=2, padx=20)
